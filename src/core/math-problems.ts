...
export const perfectSquares: Record<number, number> = {
    ...
    24: 576, 25: 625, 27: 729, 36: 1296, 40: 1600, 41: 1681,
    30: 900, 50: 2500, 60: 3600, 70: 4900, 80: 6400, 90: 8100, 100: 10000
};
...
const generateHigherPowersProblem = (difficulty: Difficulty): Problem => {
    ...
    if (difficulty === 'Medium') {
        ...
        problemSet = {
            ...
            '24^2': 576, '25^2': 625, '27^2': 729, '36^2': 1296, '40^2': 1600, '41^2': 1681
        };
    }
...
// Unified Level-3 case generator. `difficulty` lets shared techniques scale
// their number ranges (e.g., × 4 Easy uses smaller nums than × 4 Medium) and
// lets `div_4` switch between strict-mult-of-4 (Easy) and even-only (Medium,
// where the answer is either an integer or X.5). Called by both the mixed-
// mode dispatcher (`generateLevel3Problem`) and the topic-focused dispatcher
// (`generateStrategicMulDivProblem`).
const generateLevel3CaseByType = (type: string, difficulty: Difficulty): Problem => {
    switch (type) {
        case 'mul_4': {
        ...
        case 'div_4': {
            ...
            if (difficulty === 'Easy') {
                ...
                // Medium: even nums only → answer is integer or X.5
        ...
        case 'div_8': {
            // No remainder — num is a true multiple of 8
        ...
        case 'mul_11': {
            ...
            const B = num % 10;
            const sum = A + B;
            const explanation = sum <= 9
                ? `${num} × 11: place the digits as ${A}, (${A}+${B}=${sum}), ${B} → ${answer}`
                : `${num} × 11: place ${A}, (${A}+${B}=${sum}), ${B}. Since ${sum} > 9, carry the 1 → ${answer}`;
            return { question: `${num} × 11 = ?`, answer, type: 'Strategic Multiplication', explanation, inputType: 'number' };
        }
        case 'mul_12_15': {
        ...
        case 'div_12': {
            // No remainder — num is a true multiple of 12
        ...
        case 'mul_19': {
            ...
            const answer = num * 19;
            const explanation = `${num} × 19 = ${num} × (20 - 1) = ${num*20} - ${num} = ${answer}`;
            return { question: `${num} × 19 = ?`, answer, type: 'Strategic Multiplication', explanation, inputType: 'number' };
        }
        case 'mul_99': {
            const num = Math.floor(Math.random() * (50 - 11 + 1)) + 11;
            const answer = num * 99;
            const explanation = `${num} × 99 = ${num} × (100 - 1) = ${num*100} - ${num} = ${answer}`;
            return { question: `${num} × 99 = ?`, answer, type: 'Strategic Multiplication', explanation, inputType: 'number' };
        }
        case 'mul_25': {
        ...
        case 'divisibility': {
            // Mixed-mode divisibility, scaled by difficulty. Easy excludes 4
            // (last-two-digits check is a tier harder than digit-sum / ending);
            // Medium adds 4 and 8, drops 5 (5 is trivial at 4-digit scale).
            ...
            const [min, max] = difficulty === 'Easy' ? [100, 999] : [1000, 9999];
        ...
        case 'adv_div': {
            const divisors = [7, 11];
            const divisor = divisors[Math.floor(Math.random() * divisors.length)];
            const testNum = Math.floor(Math.random() * (9999 - 1000 + 1)) + 1000;
            const isDivisible = testNum % divisor === 0;
            const explanation = getDivisibilityExplanation(testNum, divisor, isDivisible);
...
const generateLevel3Problem = (difficulty: Difficulty, _history: string[]): Problem => {
    // Pedagogically tiered op pools. Easy = single-step shortcuts on small
    // numbers. Medium = multi-step shortcuts, bigger numbers, with-remainder
    // variants, and × 11. Hard = advanced techniques (× 19/25/99, squaring
    // X5, complementary mul, advanced divisibility).
...
// --- Standalone root estimation generator (used by topic routing) ---
...
const generateStrategicMulDivProblem = (difficulty: Difficulty): Problem => {
    // Mirrors mixed-mode tiering, minus the divisibility cases (those have
    // their own per-topic dispatchers).
    let ops: string[];
    if (difficulty === 'Easy') {
...
const generateTopicProblem = (topic: string, difficulty: Difficulty, scope?: string): Problem => {
    switch (topic) {
        ...
        case 'divisibility_7': return generateDivisibilityProblem_grouped([7, 11], difficulty);
